...
function diffBadgeLabel(type: RegDiffType): string {
  switch (type) {
    ...
    case 'key-removed':   return 'EXTRA KEY';
    case 'value-added':   return 'ADD';
    case 'value-removed': return 'REMOVE';
...
const FileCard: React.FC<FileCardProps> = ({ role, fileName, keyCount, onLoad, onClear }) => {
  ...
  const subtext = role === 'primary' ? 'First registry snapshot' : 'Second registry snapshot';
  ...
  return (
    <div className={diffRowClass(e.type)}>
      <div className="cmp-diff-row-top">
        ...
        {e.type === 'key-removed' && (
          <span className="cmp-diff-key-skip-note">only in the other file — left in place, keys are never removed</span>
        )}
      </div>

      {e.type === 'value-added' && (
        <div className="cmp-diff-data-row add">
          <span className="cmp-diff-data-label">Add</span>
      ...
      {e.type === 'value-removed' && (
        <div className="cmp-diff-data-row delete">
          <span className="cmp-diff-data-label">Remove</span>
      ...
      {e.type === 'value-modified' && (
        <>
          <div className="cmp-diff-data-row add">
            <span className="cmp-diff-data-label">Set to</span>
            <span className="cmp-diff-type-pill">{e.primaryType}</span>
            <span className="cmp-diff-data">{e.primaryData ?? ''}</span>
          </div>
          <div className="cmp-diff-data-row delete">
            <span className="cmp-diff-data-label">Current</span>
...
const StatsBar: React.FC<StatsBarProps> = (props) => {
  const items = [
    ...
    { label: 'Extra keys (kept)',        count: props.keysRemoved,    cls: 'delete' },
    { label: 'Values to add',            count: props.valuesAdded,    cls: 'add' },
    { label: 'Values to remove',         count: props.valuesRemoved,  cls: 'delete' },
    { label: 'Values to change',         count: props.valuesModified, cls: 'modify' },
  ].filter((i) => i.count > 0);
...
export const RegCompareDialog: React.FC<RegCompareDialogProps> = ({ onBack }) => {
  ...
  // Which uploaded file a device should be made to match. The two files are
  // symmetric snapshots, so the script can target either.
  ...
  // One diff drives BOTH the display and the script: the differences needed to
  // make a device match the chosen target. diffRegTrees(target, source) yields
  // ops that turn a device matching `source` into `target`. Toggling the target
  // reorients the diff view and the script together.
  const diffResult = useMemo(() => {
    if (!primaryTree || !secondaryTree) return null;
    const [targetTree, sourceTree] = target === 'primary'
      ? [primaryTree, secondaryTree]
      : [secondaryTree, primaryTree];
    return diffRegTrees(targetTree, sourceTree);
  }, [primaryTree, secondaryTree, target]);
          ...
          {hasDiff && (
            <>
              ...
              <div className="export-config-section" style={{ marginTop: 8 }}>
                <label className="export-config-label">Make a device match</label>
                <div className="export-tabs cmp-target-toggle">
                  <button
                    className={`export-tab${target === 'primary' ? ' active' : ''}`}
                    onClick={() => setTarget('primary')}
                  >
                    Primary
                  </button>
                  <button
                    className={`export-tab${target === 'secondary' ? ' active' : ''}`}
              ...
              {rightTab === 'diff' ? (
                <div className="cmp-diff-scroll">
                  <div className="cmp-diff-legend">
                    To make a device match <strong>{target}</strong>:
                    <span className="badge add" style={{ marginLeft: 8 }}>ADD</span> added
                    <span className="badge modify" style={{ marginLeft: 12 }}>MOD</span> value changed
                    <span className="badge delete" style={{ marginLeft: 12 }}>REMOVE</span> value removed
                    <span className="badge delete" style={{ marginLeft: 12 }}>EXTRA KEY</span> <em>only in the other file — keys are never deleted</em>
                  </div>
                ...
                <>
                  {diffResult!.stats.keysRemoved > 0 && (
                    <div className="cmp-key-skip-banner">
                      ...
                      <span>
                        <strong>{diffResult!.stats.keysRemoved} key{diffResult!.stats.keysRemoved !== 1 ? 's' : ''} only in the other file</strong> — keys are never deleted by the script, so the device won't be an exact match for those. Remove them manually if needed.
                      </span>
                    </div>
                  )}
                  <div className="confirmPage-intune-note">
                    <strong>Restore script:</strong>{' '}
                    Brings a device in line with the <strong>{target}</strong> .reg file. It first
                    backs up the affected keys on the device (see the script header) — so no prior
                    backup is needed — then applies the differences. Upload as an Intune Platform
                    Script (Devices &rarr; Scripts) or as a Remediation pair.
                  </div>
                </>
              )}
            </>
